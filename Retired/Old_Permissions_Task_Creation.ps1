...
Do {
    ...
    switch ($input1) { 
        ...
        '1' { 
            ...
            #Checking Source Global or Exchange Admin Username

            $TestPath = ".\ServerUser.txt"
            if ( $(Try { Test-Path $TestPath.trim() } Catch { $false }) ) {
                Write-Host "Exchange/Global Admin Username File Exists..."
            ...
            #Checking Source Global or Exchange Admin Password

            $TestPath = ".\ServerPass.txt"
            if ( $(Try { Test-Path $TestPath.trim() } Catch { $false }) ) {
                Write-Host "Exchange/Global Admin Password File Exists..."
